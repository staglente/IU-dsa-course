...
void findPath(vector<ll> &w_, vector<ll> &c_, vector<vector<ll>> &dp_, set<ll> &res, ll curi, ll curw){
    if(dp_[curi][curw] == 0)
        return;
    if(dp_[curi-1][curw] == dp_[curi][curw])
        findPath(w_, c_, dp_, res, curi - 1, curw);
    else{
        res.insert(curi);
        findPath(w_, c_, dp_, res, curi - 1, curw - w_[curi]);
    }
}
